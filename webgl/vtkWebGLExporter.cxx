...
{
public:
  vtkstd::string LastMetaData;
  vtkstd::map<unsigned long,unsigned long> ActorTimestamp;
  vtkstd::map<unsigned long,unsigned long> OldActorTimestamp;
  vtkstd::vector<vtkWebGLObject*> Objects;
  vtkstd::vector<vtkWebGLObject*> tempObj;
};
//*****************************************************************************

vtkStandardNewMacro(vtkWebGLExporter);
vtkCxxRevisionMacro(vtkWebGLExporter, "$Revision$");
...
void vtkWebGLExporter::parseActor2D(vtkActor2D *actor, long actorTime, long renderId, int layer, bool isWidget)
  ...
  if (dataMTime != actorTime && actor->GetVisibility())
    ...
    if (actor->GetMapper())
      {
      vtkstd::string name = actor->GetMapper()->GetClassName();
...
void vtkWebGLExporter::parseActor(vtkActor* actor, long actorTime, long rendererId, int layer, bool isWidget)
  ...
  if (mapper)
    ...
    if(dataMTime != actorTime && actor->GetVisibility())
      ...
      double bb[6];
      actor->GetBounds(bb);
      double m1 = vtkstd::max(bb[1]-bb[0], bb[3]-bb[2]); m1 = vtkstd::max(m1, bb[5]-bb[4]);
      double m2 = vtkstd::max(this->SceneSize[0], this->SceneSize[1]); m2 = vtkstd::max(m2, this->SceneSize[2]);
      if (m1 > m2)
        {
...
vtkTriangleFilter* vtkWebGLExporter::GetPolyData(vtkMapper* mapper, unsigned long& dataMTime)
{
  ...
  this->TriangleFilter->SetInput(dataset);
...
{
  double max = vtkstd::max(this->SceneSize[0], this->SceneSize[1]); max = vtkstd::max(max, this->SceneSize[2]);
  std::stringstream ss;
...
void vtkWebGLExporter::exportStaticScene(vtkRendererCollection *renderers, int width, int height, vtkstd::string path)
  {
  vtkstd::stringstream ss;
  ss << width << "," << height;
  vtkstd::string resultHTML = "<html><head></head><body onload='loadStaticScene();' style='margin: 0px; padding: 0px; position: absolute; overflow: hidden; top:0px; left:0px;'>";
  ...
  resultHTML += "var metadata = '" + vtkstd::string(metadata) + "';";
  ...
  for(int i=0; i<this->GetNumberOfObjects(); i++)
    ...
    if (obj->isVisible())
      ...
      for(int j=0; j<obj->GetNumberOfParts(); j++)
        ...
        test = vtkstd::string((const char *)output, size);
        resultHTML += "'" + test + "',\n";
        delete[] output;
        }
      }
  ...
  //Reading Renderer
  vtkstd::string line;
  vtkstd::string pp = "/source/pvweb/ParaViewWeb/WebServer/PWService/web/js/dev/";
  pp += "webglRenderer.js";
  ifstream render(pp.c_str());
  while(render.good())
    {
    vtkstd::getline(render, line);
    resultHTML += line + '\n';
    }
  render.close();

  pp = "/source/pvweb/ParaViewWeb/WebServer/PWService/web/js/dev/";
  pp += "glMatrix-0.9.5.min.js";
  ifstream glmatrix(pp.c_str());
  while(glmatrix.good())
    {
    vtkstd::getline(glmatrix, line);
    resultHTML += line + '\n';
    }
  glmatrix.close();
