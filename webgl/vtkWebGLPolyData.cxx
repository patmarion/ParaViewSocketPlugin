...
vtkCxxRevisionMacro(vtkWebGLPolyData, "$Revision$");
...
class vtkWebGLPolyData::vtkInternal
  {
public:
  vtkstd::vector<vtkWebGLDataSet*> Parts;
  vtkstd::map<long int, short> IndexMap;
  };
//*****************************************************************************
...
void vtkWebGLPolyData::GenerateBinaryData()
  ...
  if(this->Internal->Parts.size() != 0)
    {
    vtkstd::string MD5 = md5((char*)ss.str().c_str(), ss.str().size());
